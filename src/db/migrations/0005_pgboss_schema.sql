-- Schema pgboss gerado por getConstructionPlans('pgboss') do pg-boss 12.26.1
-- (decisão D15: plans aplicados como migration pelo owner; worker roda migrate:false).
-- Wrapper BEGIN/COMMIT/advisory-lock removido: o migrator do Drizzle já é transacional.
...
-- Acesso exclusivo do flashcards_service (runtime de usuário não enxerga pgboss).
GRANT USAGE ON SCHEMA pgboss TO flashcards_service;
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA pgboss TO flashcards_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA pgboss TO flashcards_service;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA pgboss TO flashcards_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO flashcards_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT USAGE, SELECT ON SEQUENCES TO flashcards_service;
